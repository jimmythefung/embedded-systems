...
const int SwitchButtonPin = 8;  // SW pin connected to arduino pin D8
...
int readJoyStick_SW() {
  return digitalRead(SwitchButtonPin);
}

const int potentiameterPin = 2;
int readPot() {
  return (uint32_t)analogRead(potentiameterPin) * 100 / 1024;
}

bool joystick_button_is_released() {
  static int buttonReleased = 0;
  static int buttonPressed = 0;
  if (digitalRead(SwitchButtonPin) == 0) {
    if (buttonPressed == 0) {
      buttonPressed = 1;
    }
  }

  if (digitalRead(SwitchButtonPin) == 1) {
    if (buttonPressed == 1) {
      buttonReleased = 1;
    }
  }
